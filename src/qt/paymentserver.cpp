...
const int MystCoin_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString MystCoin_IPC_PREFIX("MystCoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(MystCoin_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(MystCoin_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(MystCoin_IPC_CONNECT_TIMEOUT);
...
PaymentServer::PaymentServer(QApplication* parent) : QObject(parent), saveURIs(true)
{
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start MystCoin: click-to-pay handler");
    else
