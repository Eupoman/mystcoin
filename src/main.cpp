...
uint256 hashGenesisBlock("0x0000097dda74c1353ca4c2265d6d7cba5f81a17019e9587f11adede52cb4856c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MystCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MystCoin Signed Message:\n";
...
{
	// MystCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
	int64 nSubsidy = 13 * COIN;
	
	if (nHeight >= 2 && nHeight <= 51)
	{
		nSubsidy = 224000000 * COIN;
	}

	// Subsidy is cut in half every 64000 blocks, 
	nSubsidy >>= (nHeight / 64000);

	return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60; // MystCoin: 1 minute
static const int64 nTargetSpacing = 90; // 90 sec
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
	...
	// MystCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
	...
	// was causing -reindex to crash MystCoind
	if (hash != hashGenesisBlock) {
...
{
	// MystCoin: temporarily disable v2 block lockin until we are ready for v2 transition
	return false;
...
bool LoadBlockIndex()
{
	if (fTestNet)
	{
		...
		hashGenesisBlock = uint256("0x0000097dda74c1353ca4c2265d6d7cba5f81a17019e9587f11adede52cb4856c");
	}
...
bool InitBlockIndex() {
	...
	if (!fReindex) {
		...
		const char* pszTimestamp = "France Creates Working Group for Cryptocurrency Regulation";
		CTransaction txNew;
		txNew.vin.resize(1);
		txNew.vout.resize(1);
		txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
		txNew.vout[0].nValue = 13 * COIN;
		txNew.vout[0].scriptPubKey = CScript() << ParseHex("028184710fa25aad5023690c805b849c8f13f8d45b88857dbcbc8bc4a8e4d3eb4b10f4d4604fc88dce601aaf0f760216fe1b518a0b4acf21b179c47070ac7b03a6") << OP_CHECKSIG;
		...
		// Merkle root
		assert(block.hashMerkleRoot == uint256("0xfaa34cff536eefa61b0ad7b18660254971a6e5b92600990b5cb62d82f783287e"));
...
unsigned char pchMessageStart[4] = { 0xc4, 0x7a, 0xb3, 0xe2 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
	...
	// Found a solution
	{
		...
		if (pblock->hashPrevBlock != hashBestChain)
			return error("MystCoinMiner : generated block is stale");
		...
		if (!ProcessBlock(state, NULL, pblock))
			return error("MystCoinMiner : ProcessBlock, block not accepted");
	}

	return true;
}

void static MystCoinMiner(CWallet *pwallet)
{
	printf("MystCoinMiner started\n");
	SetThreadPriority(THREAD_PRIORITY_LOWEST);
	RenameThread("MystCoin-miner");
	...
	try {
	 ...
	 while (vNodes.empty())
		...
		printf("Running MystCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
	...
	{
		printf("MystCoinMiner terminated\n");
		throw;
	}
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
	...
	for (int i = 0; i < nThreads; i++)
		minerThreads->create_thread(boost::bind(&MystCoinMiner, pwallet));
}
